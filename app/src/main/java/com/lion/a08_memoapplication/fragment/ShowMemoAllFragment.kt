    ...
    fun settingToolbar(){
        fragmentShowMemoAllBinding.apply {
            ...
            toolbarShowMemoAll.setOnMenuItemClickListener {
                when(it.itemId){
                    R.id.toolbar_show_memo_all_menu_search -> {

                        val dataBundle = Bundle().apply {
                            putString("MemoName", arguments?.getString("MemoName"))
                            if (arguments?.getString("MemoName") == MemoListName.MEMO_NAME_ADDED.str) {
                                putInt("categoryIdx", arguments?.getInt("categoryIdx") ?: -1)
                            }
                        }

                        // 검색 화면으로 이동한다.
                        mainActivity.replaceFragment(FragmentName.SEARCH_MEMO_FRAGMENT,true,true,dataBundle)
                    }
                }
                true
            }
