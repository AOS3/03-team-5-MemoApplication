...
class MemoRepository {
    companion object {
        ...
        fun selectMemoDataAllByFavorite(
            context: Context,
            memoIsFavorite: Boolean
        ...
        fun selectMemoDataAllBySecret(
            context: Context,
            memoIsSecret: Boolean
        ): MutableList<MemoModel> {
            // 데이터를 가져온다.
            val memoDataBase = MemoDataBase.getInstance(context)
        ...
        fun selectMemoDataAllByCategoryIdx(
            context: Context,
            categoryIdx: Int
        ): MutableList<MemoModel> {
            // 데이터를 가져온다.
        ...
        // 메모를 제목이나 내용으로 검색하는 메서드
        fun searchMemoByTitleOrText(context: Context, query: String, isFavorite: Boolean? = null, categoryIdx: Int? = null): MutableList<MemoModel> {
            val memoDataBase = MemoDataBase.getInstance(context)
            val keywords = query.trim().split("\\s+".toRegex())  // 공백으로 단어 분리

            val memoList = mutableListOf<MemoModel>()

            // 첫 번째 키워드로 검색 (LIKE 쿼리 사용)
            val memoVoList =
                memoDataBase?.memoDao()?.searchMemoByTitleOrText("%${keywords.first()}%")

            memoVoList?.forEach { vo ->
                // 비밀메모는 검색 결과에서 제외
                if (vo.memoIsSecret) return@forEach

                // 필터 조건에 맞는지 체크
                if ((isFavorite == null || vo.memoIsFavorite == isFavorite) &&
                    (categoryIdx == null || vo.memoCategoryIdx == categoryIdx)
                ) {

                    // 모든 키워드가 제목이나 내용에 포함되어 있는지 확인
                    if (keywords.all { keyword ->
                            vo.memoTitle.contains(keyword, ignoreCase = true) ||
                                    vo.memoText.contains(keyword, ignoreCase = true)
                        }) {
                        val memoModel = MemoModel(
                            memoIdx = vo.memoIdx,
                            memoTitle = vo.memoTitle,
                            memoText = vo.memoText,
                            memoIsSecret = vo.memoIsSecret,
                            memoIsFavorite = vo.memoIsFavorite,
                            memoPassword = vo.memoPassword,
                            memoCategoryIdx = vo.memoCategoryIdx
                        )
                        memoList.add(memoModel)
                    }
                }
